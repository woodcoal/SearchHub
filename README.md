...
（QPS 与配额按各家免费额度设置，无官方明确说明的按 5 取）：

| 供应商 | 全局 QPS | 日配额 | 月配额 | 总配额 | 依据 |
|---|---|---|---|---|---|
| `anysearch` | **20** | **1000** | 不限 | 不限 | 免费额度：QPS 20、日限额 1000 |
| `exa` | **10** | 不限 | **1000** | 不限 | 免费额度：QPS 10、月限额 1000 |
| `serper` | **5** | 不限 | 不限 | **2500** | QPS 官方未明确，取 5；免费一次性 2500 credits 且不按月重置 |
| `tavily` | **5** | 不限 | **1000** | 不限 | QPS 官方未明确，取 5；免费 1000 credits/月（basic 搜索 1 credit） |

- 这些是**供应商级默认值**，密钥里留空的字段继承它们；也可以在「供应商配置」里按实际套餐改
- **日配额只给 anysearch 设置**（它明确按日限额），其余三家一律「不限」，只受 QPS 与月/总配额约束
- 升级已有数据文件时会按 `quotaSeedVersion` 做迁移：**只有当前值仍等于上一版内置默认值时才覆盖**，
  你在界面上手动改过的值一律保留
